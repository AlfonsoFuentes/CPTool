...
namespace CPTool.ApplicationCQRS.Features.Suppliers.Queries.Export
{
    public class ExportSuppliersQueryHandler : IRequestHandler<ExportSuppliersQuery, ExportBaseResponse>
    {
        ...
        public ExportSuppliersQueryHandler( ICsvExporter csvExporter, IExcelService excelService, IPDFService pdfService)
        {
        ...
        public async Task<ExportBaseResponse> Handle(ExportSuppliersQuery request, CancellationToken cancellationToken)
        {
            ...
            result.ExportFileName = $"Supplier_{DateTime.Now.ToString()}";
             result.ExportFileName += $"_{DateTime.Now.ToString()}.{request.Type}";
            

            if (request.Type == "xlsx")
            {
                result.Data = await _excelService.ExportAsync(request.List, request.Dictionary, "Supplier");
