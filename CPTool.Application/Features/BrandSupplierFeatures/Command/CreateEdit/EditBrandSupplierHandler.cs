...
{
    internal class EditBrandSupplierHandler : EditBaseHandler<EditBrandSupplier, BrandSupplier>, IRequestHandler<EditBrandSupplier, Result<int>>
    {


        public EditBrandSupplierHandler(IUnitOfWork unitofwork, IMapper mapper, IEmailService emailService, ILogger<EditBrandSupplier> logger)
        ...
        public override async Task<Result<int>> Handle(EditBrandSupplier command, CancellationToken cancellationToken)
        {
            ...
            {
                var result = await SolveBrand(command.Brand!);
                if (result.Succeeded)
                {
                    command.Brand = result.Data;
                }
            ...
            {
                var result = await SolveSupplier(command.Supplier!);
                if (result.Succeeded)
                {
                    command.Supplier = result.Data;
                }
            }
            if (command.Brand!.Id != 0 && command.Supplier!.Id != 0)
            {
                var exist = await _unitOfWork.Repository<BrandSupplier>().GetAsync(
                    x => x.BrandId == command.Brand!.Id && x.SupplierId == command.Supplier!.Id);
