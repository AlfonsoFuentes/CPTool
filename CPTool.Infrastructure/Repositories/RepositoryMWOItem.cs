    ...
    public class RepositoryMWOItem : RepositoryBase<MWOItem>, IRepositoryMWOItem
    {
        ...
        public async Task<MWOItem> GetMWOItemIdAsync(int id)
        {
            var result = await dbcontext!.MWOItems!
                 ...
                 .Include(x => x.StructuralItem)
                 .Include(x => x.ElectricalItem)
                 .Include(x => x.PaintingItem)
                 .Include(x => x.EHSItem)
                 .Include(x => x.TaxesItem)
                 .Include(x => x.TestingItem)
                 .Include(x => x.ContingencyItem)
                 .Include(x => x.EngineeringCostItem)
