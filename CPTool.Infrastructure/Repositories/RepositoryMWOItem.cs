...
namespace CPTool.Infrastructure.Repositories
{
    public class RepositoryMWOItem : RepositoryBase<MWOItem>, IRepositoryMWOItem
    {
        ...
        //public override async Task<MWOItem> GetByIdAsync(int id)
        //{
        //    //var result = await dbcontext.MWOItems.FindAsync(id);
        //    //     .Include(x => x.PurchaseOrderItems!).ThenInclude(x => x!.PurchaseOrder)
        //    //     .Include(x => x.PurchaseOrderItems!).ThenInclude(x => x!.PurchaseOrder).ThenInclude(x => x!.pBrand)
        //    //     .Include(x => x.PurchaseOrderItems!).ThenInclude(x => x!.PurchaseOrder).ThenInclude(x => x!.pSupplier)
        //    //     .Include(x => x.Chapter)
        ...
        //    //     .Include(x => x.EquipmentItem).ThenInclude(t => t.eBrand)
        //    //     .Include(x => x.EquipmentItem).ThenInclude(t => t.eSupplier)
        ...
        //    //      .Include(x => x.InstrumentItem).ThenInclude(t => t!.iBrand)
        //    //     .Include(x => x.InstrumentItem).ThenInclude(t => t!.iSupplier)
        ...
        {
            var result = await dbcontext!.MWOItems!.AsNoTrackingWithIdentityResolution()
                 //.Include(x=>x.MWO)
                 .Include(x => x.PurchaseOrderItems!)/*.ThenInclude(x => x.PurchaseOrder)*/
                 .Include(x => x.PurchaseOrderItems!).ThenInclude(x => x!.PurchaseOrder).ThenInclude(x => x!.pBrand)
                 .Include(x => x.PurchaseOrderItems!).ThenInclude(x => x!.PurchaseOrder).ThenInclude(x => x!.pSupplier)
                 ...
                 .Include(x => x.EngineeringCostItem)
                 .Include(x => x.EquipmentItem)
                 .Include(x => x.EquipmentItem).ThenInclude(x => x!.eInnerMaterial)
                  .Include(x => x.EquipmentItem).ThenInclude(x => x!.eOuterMaterial)
                  .Include(x => x.EquipmentItem).ThenInclude(y => y!.eEquipmentType)
                  .Include(x => x.EquipmentItem).ThenInclude(y => y!.eEquipmentTypeSub)
                  .Include(x => x.EquipmentItem).ThenInclude(x => x!.eProcessFluid)
                  .Include(x => x.EquipmentItem).ThenInclude(x => x!.eProcessCondition)
                  ...
                  .Include(x => x.EquipmentItem).ThenInclude(x => x!.Nozzles)
                 .Include(x => x.InstrumentItem)
                 .Include(x => x.InstrumentItem).ThenInclude(x => x!.iInnerMaterial)
                  .Include(x => x.InstrumentItem).ThenInclude(x => x!.iOuterMaterial)
                  .Include(x => x.InstrumentItem).ThenInclude(x => x!.iProcessCondition)
                  .Include(x => x.InstrumentItem).ThenInclude(x => x!.iProcessFluid)
                  .Include(x => x.InstrumentItem).ThenInclude(x => x!.iProcessCondition).ThenInclude(x => x!.SpecificCp)
                  .Include(x => x.InstrumentItem).ThenInclude(x => x!.iProcessCondition).ThenInclude(x => x!.ThermalConductivity)
